...
The toolchain resolves `.planning/github-issues/ISSUE_PLAN.md` PQ-1. ARCHITECTURE.md and SECURITY.md already fixed Node.js, Vue.js, REST over HTTPS, relational persistence, Terraform on AWS, and the JWT session format; these choices sit within those constraints and do not revisit them.

- **Language, package manager, build tooling:** TypeScript, npm with workspaces, and Vite as the client build tool. The runtime is Node.js 24, the active LTS line, pinned by a root `engines` field (`>=24 <25`) and an `.nvmrc`; CI and the Fargate container base image use the same major version, and moving to a newer LTS is an explicit decision recorded here (2026-08-03). Production and CI installs use `npm ci` against the committed `package-lock.json` (DEP-7).
- **Client rendering, routing, and styling:** A single-page application with `vue-router`. There is no server-rendering tier — ARCHITECTURE.md defines five components and adding one would be an architecture change, not a build-tool choice. Vite emits static assets and the server stays a pure REST API, which keeps DR-1 intact and makes SEC-HTTP-2's no-inline-script CSP reachable. Styling is plain CSS custom properties plus scoped single-file-component styles: DESIGN.md already fixes the tokens, and they map one-to-one onto custom properties, so no CSS framework is introduced (DEP-2). A component library MUST NOT be adopted without an explicit decision — it would import a competing visual system and accessibility posture over DESIGN.md's WCAG 2.2 AA rules.
...
- **RDBMS engine and migration tooling:** PostgreSQL, with Drizzle ORM and drizzle-kit. Migrations may contain raw SQL, which is how below-application enforcement — the append-only audit privileges (SEC-LOG-2, SEC-LOG-7) and any row-level security policies a future decision introduces — is expressed. RLS itself is not currently required by any SECURITY.md rule.
- **Test framework and runner commands:** Vitest for unit and component tests across both workspaces; Playwright with axe-core for end-to-end and WCAG 2.2 AA accessibility testing. DESIGN.md's commitments that need a real browser — 200% zoom, 320px reflow without horizontal scroll, keyboard operability with no traps, visible focus, reduced motion — are Playwright's responsibility, not Vitest's. Runner commands: TO BE DECIDED — the scripts do not exist until the scaffolding lands. Code-coverage threshold: 90% line and branch coverage per workspace, enforced by Vitest once test suites exist; it may be raised but MUST NOT be lowered without a recorded decision (2026-08-03).
...
- **CI platform and pipeline:** GitHub Actions is the platform, using OIDC federation to a distinct AWS IAM role per environment account (SEC-CICD-1). The merge-blocking gate set (SEC-CICD-4): lint + typecheck, Vitest, Playwright + axe, osv-scanner dependency audit, gitleaks secret scan, checkov IaC scan, and the authorization test suite. Interim posture (2026-08-03): the scaffolding PR (REQ-BUILD-010) ships a minimal GitHub Actions workflow with lint + typecheck, Vitest, gitleaks, and osv-scanner as merge-blocking checks from the first PR; Playwright + axe, checkov, and the authorization suite each become blocking the moment their subject exists; REQ-INFRA-070 completes and formalizes the full set and branch protection. Staging auto-deploys from `main`; production requires manual approval. AWS topology (SECURITY.md SQ-7 RESOLVED): dev/staging/production as separate accounts; ECS Fargate API, S3 + CloudFront client, RDS PostgreSQL Multi-AZ, Amazon Bedrock (zero-retention) for AI inference, AWS Secrets Manager for secrets and signing keys.
...
- Workflow (recorded 2026-08-03): branches are named `<type>/<short-slug>` with types `specs`, `feat`, `fix`, `infra`, `backlog`; every change lands via a pull request; merges are squash-only; the merge-blocking CI gates (SEC-CICD-4, including the interim posture above) are the required status checks; no human reviewer is required while the repository has a single maintainer — revisit when a second maintainer joins; the PR body names the requirement, architecture, security, and design identifiers the change traces to. Release process: TO BE DECIDED — nothing exists to release yet.
