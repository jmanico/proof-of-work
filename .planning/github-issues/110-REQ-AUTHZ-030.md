...
- **Out of Scope**: The verification workflow itself, which depends on `REQUIREMENTS.md` OQ-10 (re-verification after edit) and OQ-16 (dual control) and is blocked; how admin accounts are provisioned (`SECURITY.md` SQ-12); customization of a published plan by a subscriber, which is a distinct permitted behavior (REQ-CUSTOM-010).
