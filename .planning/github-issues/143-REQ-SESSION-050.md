...
- **Assumptions**: One first-party browser client and no documented cross-origin consumer (`SECURITY.md` SQ-6), so no third-party origin needs a header-based alternative.
- **Out of Scope**: Session record semantics and resolution (REQ-SESSION-030); logout and revocation (REQ-SESSION-040); token signature and claims (REQ-SESSION-010, REQ-SESSION-020); TLS termination and the wider security header set (REQ-PLATFORM-040); CORS posture (SEC-HTTP-3, blocked by SQ-6); signing key storage (SEC-SESSION-7, blocked by SQ-7); the `SameSite` value and cookie lifetime, `TO BE DECIDED` under SQ-3.
...
1. **AC-01 — Expected behavior**: Given a successful authentication, when the session is issued, then it is set as a cookie carrying `HttpOnly`, `Secure`, and a `SameSite` attribute, and the response body contains no token value.
...
- **Alerting**: TO BE DECIDED — a spike in forgery denials is a meaningful signal, but thresholds are blocked by `SECURITY.md` SQ-3.
...
- **Required Test Environment**: A TLS-terminating configuration, a hostile-origin fixture for cross-origin tests, and Playwright for real-browser storage and history inspection; Vitest as the runner. Deployment topology remains TO BE DECIDED (`SECURITY.md` SQ-7).
...
- **Constraints**: Node.js runtime with Fastify; Vue single-page application built by Vite (`CLAUDE.md`). The `SameSite` value and cookie lifetime are `TO BE DECIDED` under SQ-3, so they MUST be named constants rather than literals scattered through the code. Because the client is a single-page application on the same origin, no cross-origin exemption is needed (SQ-6).
...
- **Open Decisions**: The `SameSite` value and cookie lifetime (`SECURITY.md` SQ-3). The specific anti-forgery pattern is an implementation choice within this issue, but it MUST satisfy AC-04 against a genuine hostile origin rather than only a same-origin test harness.
