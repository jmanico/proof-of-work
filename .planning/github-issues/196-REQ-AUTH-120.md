...
- **Out of Scope**: Enrolment, disablement, and code issuance (REQ-AUTH-110); password authentication (REQ-AUTH-100); passkey authentication as a primary factor for privileged roles (REQ-AUTH-020); throttling (REQ-AUTH-060); session creation mechanics (REQ-SESSION-030); TOTP drift window and code count, `TO BE DECIDED` under `SECURITY.md` SQ-3.
- **Design Traceability**: `DESIGN.md` — Components → Inputs (numeric second-factor entry with a visible persistent label), Buttons (busy state preventing repeat submission), Form feedback and errors, and Accessibility (the challenge must be completable by keyboard with focus managed onto the input).
...
- **Alerting**: TO BE DECIDED — repeated failed second factors and recovery-code use are both meaningful signals, but thresholds are blocked by `SECURITY.md` SQ-3.
...
- **Constraints**: Node.js runtime with Fastify; Vue single-page application (`CLAUDE.md`). TOTP drift window and in-progress attempt expiry are `TO BE DECIDED` (`SECURITY.md` SQ-3) and MUST be named constants.
...
- **Open Decisions**: TOTP drift window, in-progress attempt expiry, and recovery-code count (`SECURITY.md` SQ-3). Whether a user with several enrolled factors may choose which to present depends on the multi-factor question left provisional in REQ-AUTH-110.
