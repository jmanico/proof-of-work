...
- **Out of Scope**: Every functional requirement — this issue implements no `FR-*` and adds no product behavior. Client application code and interface work, which belong to REQ-PLATFORM-010 through -030 and the REQ-CATALOG issues; this issue stands up the `apps/web` workspace and its Vite build but ships no view, route, or component. The full SEC-CICD-4 merge-blocking gate set and branch-protection configuration, whose content is fixed (`SECURITY.md` SEC-CICD-4, SQ-7 RESOLVED) and whose completion is REQ-INFRA-070, with pipeline identities and deployment flow in REQ-INFRA-010 — this issue ships only the interim minimal workflow of AC-07 (lint + typecheck, Vitest, gitleaks, osv-scanner), per the interim posture recorded in `CLAUDE.md` (2026-08-03). Terraform content under `infra/`, which receives a directory and nothing else — infrastructure delivery is REQ-INFRA-010 through REQ-INFRA-060. Database *schema* — tables belong to REQ-AUTH-010, REQ-AUDIT-010, REQ-PLAN-010, REQ-PLAN-020, and REQ-CUSTOM-030, and this issue delivers only the mechanism that runs them. Secret and configuration management (SEC-SECRET-2 — AWS Secrets Manager per SQ-7, delivered by REQ-INFRA-030). Any dependency beyond the recorded toolchain, which REQ-PIPE-010 governs.
...
6. **AC-06 — Additional criterion**: Given a disposable PostgreSQL instance, when the migration command applies all migrations to an empty database and is then invoked a second time, then the first run succeeds and the second is a clean no-op reporting nothing to apply — proving the mechanism the schema-bearing issues depend on. Migrations are forward-only: drizzle-kit generates no down migrations, recovery from a bad migration is a new forward migration, and a failed migration leaves the database in its prior state (Failure Behavior). *(Amended 2026-08-03: the original reverse-migration criterion conflicted with the fixed forward-only toolchain.)*
7. **AC-07 — Additional criterion**: Given the repository on GitHub, when a pull request is opened, then a GitHub Actions workflow runs lint + typecheck, Vitest, gitleaks, and osv-scanner as required, merge-blocking status checks — the interim subset of the SEC-CICD-4 gate set that needs no AWS resources. The remaining gates (Playwright + axe, checkov, the authorization test suite) become blocking as their subjects land, and REQ-INFRA-070 completes and formalizes the full set and branch protection. *(Added 2026-08-03: closes the unguarded-merge window between scaffolding and REQ-INFRA-070.)*
...
- **Integration Tests**: Clean-checkout bootstrap exercising every documented command; migration applied and then re-invoked as a no-op against a disposable PostgreSQL instance (AC-06); a deliberately stale lockfile causing `npm ci` to fail (AC-02); the interim CI workflow observed running and blocking on a deliberately failing check (AC-07).
