...
- **Rationale**: Six issues have no in-plan predecessor and REQ-PIPE-010's precondition is now satisfied, yet none of the 49 drafted issues creates the manifest, the migration mechanism, the test harness, or the lint configuration they all presuppose. Six issues name a schema migration as their delivery vehicle and no issue establishes migrations. Two security rules — SEC-RENDER-1 and SEC-INPUT-5 — name a lint rule as their *verification method*, so until the lint configuration exists those rules have no stated means of verification. This issue supplies that substrate once, so no feature issue has to invent it and diverge.
...
- **Out of Scope**: Every functional requirement — this issue implements no `FR-*` and adds no product behavior. Client build tooling, bundler, dev server, styling approach, and routing strategy, all `TO BE DECIDED`, which bounds `apps/web` to a workspace with language, lint, and test configuration only (see Open Decisions). CI workflow definitions and the merge-blocking gate set (`SECURITY.md` SQ-7, SEC-CICD-4). Terraform content under `infra/`, which receives a directory and nothing else (SQ-7). Database *schema* — tables belong to REQ-AUTH-010, REQ-AUDIT-010, REQ-PLAN-010, REQ-PLAN-020, and REQ-CUSTOM-030, and this issue delivers only the mechanism that runs them. Secret and configuration management (SEC-SECRET-2, blocked by SQ-7). Any dependency beyond the recorded toolchain, which REQ-PIPE-010 governs.
...
- **Architecture Traceability**: `ARCHITECTURE.md` — all four components receive their directory: Browser Client (`apps/web`), REST API Application (`apps/api`), Identity and Session Handling (a module inside `apps/api`, preserving DR-8 because module-versus-service is still open), Relational Persistence (`db/migrations`). DR-1 (the client depends on the API only, never on persistence), DR-5 (persistence reachable only from the API and Identity), DR-6 (one-directional dependencies, no cycles), DR-8 (unresolved decisions sit behind component interfaces and are not hard-coded).
...
- **OWASP ASVS 5.0.0**: TO BE DECIDED — not verified against `REF-ASVS-5` in this session; `SECURITY.md` SQ-10 leaves the target and its verifier open.
...
6. **AC-06 — Additional criterion**: Given a disposable PostgreSQL instance, when the migration command runs forward and then in reverse on an empty database, then it succeeds in both directions and leaves no residual objects — proving the mechanism the six schema-bearing issues depend on.
...
- **Alerting**: N/A — no runtime service is introduced.
...
- **Required Test Environment**: A clean checkout with no prior npm cache or `node_modules`; a disposable PostgreSQL instance for the migration round trip; Vitest as the runner.

## Dependencies

- **Upstream Requirements**: None — this is the root of the dependency graph.
- **Downstream Requirements**: All 48 leaf issues. Directly and immediately: REQ-API-010, REQ-API-030, REQ-SESSION-010, REQ-AUDIT-040, REQ-PLATFORM-040, and REQ-PIPE-010, which are the issues with no other in-plan predecessor. REQ-AUTH-010, REQ-AUDIT-010, REQ-PLAN-010, REQ-PLAN-020, REQ-API-030, and REQ-CUSTOM-030 depend specifically on the migration mechanism.
...
- **Constraints**: TypeScript with npm workspaces, Fastify, PostgreSQL with Drizzle ORM and drizzle-kit, Vitest, ESLint flat config with Prettier, and the `apps/api` / `apps/web` / `packages/shared` / `db/migrations` / `infra` layout (`CLAUDE.md`). Client build tooling is `TO BE DECIDED`, so `apps/web` receives language, lint, and test configuration but no bundler, dev server, or routing setup; the client half of the plan stays blocked until that resolves. `packages/shared` carries contract shape only — a rule that lives there rather than in `apps/api` violates DR-2.
- **Prohibited Approaches**: Scaffolding anything that resolves an open question — no bundler choice, no styling framework, no session transport, no schema tables, no CI workflow, no authentication stub. Creating placeholder route handlers or model classes that later issues must delete. Enforcing the dependency direction by documentation or review convention rather than by a mechanism that fails the build (DR-2's reasoning applies to structure as much as to business rules). Committing a `.env`, key, or connection string with real values. Adding a dependency outside the recorded toolchain without the REQ-PIPE-010 justification. Disabling a lint rule to make the skeleton pass.
- **Implementation Guidance**: The dominant risk is scaffolding too much. A skeleton that quietly picks a bundler, invents a directory convention, or stubs an endpoint converts an open question into a fait accompli that no reviewer will revisit — which is exactly what `CLAUDE.md` forbids. Prefer an empty workspace that fails loudly over a populated one that presumes. Wire the two security lint rules before any code exists to trip them, so they are known to work rather than assumed to.
...
- **Open Decisions**: Client build tooling, bundler, styling approach, and routing strategy (`CLAUDE.md`; `ARCHITECTURE.md` Browser Client) — until these resolve, `apps/web` is a configured workspace that cannot yet build a running client, and REQ-PLATFORM-010, REQ-PLATFORM-020, REQ-PLATFORM-030, REQ-CATALOG-010, REQ-CATALOG-020, and REQ-CATALOG-030 remain blocked on it. The code-coverage threshold is unset. The CI workflow and merge-blocking gate set are blocked by `SECURITY.md` SQ-7. Whether Identity and Session Handling becomes a separate deployable is left open per DR-8; it is scaffolded as a module inside `apps/api` so either outcome remains available.
