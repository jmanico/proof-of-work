...
- **Out of Scope**: Invitation and first enrolment (REQ-AUTH-140); passkey registration mechanics (REQ-AUTH-030); passkey authentication (REQ-AUTH-020); subscriber recovery (REQ-AUTH-130, REQ-AUTH-120); operational break-glass, which SEC-OPS-1 requires be deny-by-default and SQ-13 leaves open; vetting and deprovisioning, open under SQ-12.
- **Design Traceability**: `DESIGN.md` — Components → Buttons and Form feedback. Refusing an action because it would breach a minimum must explain which minimum and how to satisfy it, since the user can act on that; it discloses nothing sensitive.
...
- **Alerting**: TO BE DECIDED — dropping to the minimum is worth alerting on, but destinations are blocked by `SECURITY.md` SQ-3 and SQ-11.
...
- **Failure Impact**: Without the minimums, one lost device makes the plan library permanently unadministrable, and the only remaining route is direct database access — precisely the operator path SEC-OPS-1 requires be deny-by-default and SQ-13 leaves undefined.

## Implementation Notes

- **Constraints**: Node.js runtime with Fastify; PostgreSQL with Drizzle ORM (`CLAUDE.md`). The minimums are fixed at two by FR-2.15 and are not tunable without a requirement change; express them as named constants so the intent is legible, not so they can be lowered in configuration.
...
- **Open Decisions**: Whether an operational break-glass exists as a last resort if both minimums are somehow breached remains open under `SECURITY.md` SQ-13 and SEC-OPS-1, and MUST NOT be invented here. Deprovisioning of privileged accounts is open under SQ-12 and interacts directly with AC-04 — removing a departing admin requires a replacement first, which should be raised when SQ-12 is taken up.
