...
- **Out of Scope**: Password policy and breached-password refusal, which belong to registration and reset (REQ-AUTH-080, REQ-AUTH-130); throttling (REQ-AUTH-060); the authentication flow itself (REQ-AUTH-100); passkey credential material, which is REQ-AUTH-020 and REQ-AUTH-030; machine-held high-entropy tokens — password-reset, email-verification, and enrolment tokens — which SEC-AUTHN-11 (2026-08-03) stores with HMAC-SHA-256 under a secret-store key, not with this function.
...
5. **AC-05 — Additional criterion**: Given the credential module, when its parameters are reviewed, then Argon2id memory, iteration, and parallelism values are named constants with a documented tuning basis, not literals at the call site — concretely 64 MiB memory, 3 iterations, parallelism 1, tuned to the 1 vCPU / 2 GB Fargate task and re-evaluated on any resize (SEC-AUTHN-5, `SECURITY.md` SQ-7 RESOLVED).
...
- **Alerting**: Threshold alerts on credential-path anomalies (SEC-LOG-4 events) route to the security lead as SEC-OPS-2 detection inputs (`SECURITY.md` SQ-3, SQ-11 RESOLVED).
...
- **Downstream Requirements**: REQ-AUTH-080 (registration), REQ-AUTH-100 (authentication), REQ-AUTH-130 (password reset — the new password credential; the reset token itself is HMAC-SHA-256 per SEC-AUTHN-11), REQ-AUTH-120 (user-typed MFA recovery codes are stored with this same function per SEC-AUTHN-11)
...
- **Constraints**: Node.js runtime with Fastify; PostgreSQL with Drizzle ORM (`CLAUDE.md`). Concrete Argon2id parameters are fixed (`SECURITY.md` SQ-7 RESOLVED; SEC-AUTHN-5): memory 64 MiB, iterations 3, parallelism 1, shipped as named constants whose documented tuning basis is the 1 vCPU / 2 GB Fargate task, re-evaluated on any resize.
...
- **Open Decisions**: None. Argon2id memory, iteration, and parallelism values are fixed (`SECURITY.md` SQ-7 RESOLVED). Whether a pepper held in the secret store is used in addition to the salt is not specified by any source document; it MUST NOT be added unilaterally, since it interacts with the AWS Secrets Manager rotation model (SEC-SECRET-2, SQ-7 RESOLVED) and would be a spec change.
