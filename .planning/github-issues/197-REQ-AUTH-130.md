...
- **Out of Scope**: MFA recovery when the second factor itself is lost, which is recovery-code redemption in REQ-AUTH-120; privileged account recovery, which is REQ-AUTH-150 and never uses this path; password change by an already-authenticated user, which is a distinct operation not drafted here; throttling (REQ-AUTH-060); token lifetime, `TO BE DECIDED` under `SECURITY.md` SQ-3.
- **Design Traceability**: `DESIGN.md` — Components → Inputs, Form feedback and errors. Policy failures on the new password are format problems and may be specific; whether an address is registered may never be revealed, so the request step always reports the same outcome.
...
- **Alerting**: TO BE DECIDED — a reset is a meaningful account-takeover signal, but thresholds and destinations are blocked by `SECURITY.md` SQ-3 and SQ-11.

## Test Strategy

- **Unit Tests**: Token generator uses the secure generator; verifier rejects expired, consumed, forged, and cross-account tokens; policy evaluation runs on the new password; an unreadable MFA state yields refusal.
...
- **External Dependencies**: The mail delivery mechanism introduced by REQ-AUTH-090; no new dependency, and no health data crosses it (FR-9.8, SEC-TB-3).
...
- **Open Decisions**: Token lifetime (`SECURITY.md` SQ-3). Whether the user is notified by email that a reset occurred is not specified by any source document; it is common practice and would help detect takeover, but it MUST be recorded as a decision rather than added silently.
