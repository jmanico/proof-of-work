...
- **Statement**: The system MUST verify control of a registered email address by a single-use, time-limited token before that account may record any health data or before the address is relied upon in any recovery flow, and MUST refuse every health-data write while the address is unverified.
...
- **Out of Scope**: Registration itself (REQ-AUTH-080); recovery flows that consume verification state (REQ-AUTH-130); throttling of the resend endpoint, provided by REQ-AUTH-060; the email-change path, which `REQUIREMENTS.md` FR-9.5 touches and REQ-PRIVACY-030 excludes, and which is a separate requirement not drafted here; token lifetime and resend interval, `TO BE DECIDED` under `SECURITY.md` SQ-3; the mail transport mechanism, which is a deployment concern under SQ-7.
- **Design Traceability**: `DESIGN.md` — Components → Empty states and Form feedback. An account in the unverified state needs a persistent, non-blocking indication of what is unavailable and how to resolve it, announced politely rather than as an error.
...
- **Personal or Regulated Data**: Personal Data — email address and verification token; the gate protects Health Data
...
2. **AC-02 — Boundary or failure behavior**: Given an account whose address is unverified, when any health-data write is attempted — body weight, workout, consent to health-data processing, or any other logging path — then it is refused with a message naming verification as the requirement, and nothing is persisted.
...
4. **AC-04 — Additional criterion**: Given a resend request, when it is submitted for a registered address and for an unregistered one, then the responses are indistinguishable in content, status, and timing, and resends are throttled per REQ-AUTH-060 (SEC-AUTHN-3, TM-I-4).
...
- **Security Tests**: Token replay, cross-account, and expiry suites (AC-03); differential response and timing on resend (AC-04); route inventory over health-data write paths (AC-06); log assertion that no token value is emitted.
...
- **Downstream Requirements**: REQ-PRIVACY-010 (consent capture is a health-data write and sits behind this gate), REQ-PROGRESS-010, REQ-PROGRESS-020, REQ-AUTH-130 (recovery depends on a verified address)
- **External Dependencies**: A mail delivery mechanism. This is the one outbound path the system needs; it carries no health data, so SEC-TB-3 and FR-9.8 are not breached, but its introduction should be recorded against SEC-EXT-1 when the deployment topology is chosen (SQ-7).
...
- **Constraints**: Node.js runtime with Fastify; PostgreSQL with Drizzle ORM; Vue single-page application (`CLAUDE.md`). Token lifetime and resend interval are `TO BE DECIDED` (`SECURITY.md` SQ-3) and MUST be named constants. The mail transport is undecided under SQ-7 and MUST sit behind an internal interface per DR-7, so choosing it later does not touch this logic.
...
- **Open Decisions**: Token lifetime and resend interval (`SECURITY.md` SQ-3). Mail transport (SQ-7). Whether an unverified account should expire after some period is not specified by any source document and MUST NOT be invented; if desired it is a new requirement.
