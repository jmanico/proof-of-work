...
- **Primary users / actors:** Subscriber (end user who follows plans and logs progress); Admin (authors, cites, verifies, and publishes plan content)
...
- **Authentication / roles:** Account required. Email + password minimum, with optional user-enabled MFA. Roles: subscriber, admin
- **Regulatory or privacy constraints:** GDPR, CCPA, and HIPAA obligations apply to user health data; a medical disclaimer stating the content is not medical advice must be shown
...
- **FR-2.7** The system MUST assign every account exactly one of the roles `subscriber` or `admin`.
...
- **OQ-3** HIPAA applies only to covered entities and their business associates. What is the covering relationship here, and does it require a BAA, breach-notification workflow, or specific access controls beyond FR-9.7?
...
- **OQ-12** Is offline use required for logging entries without a connection? Not selected, so assumed out of scope.
