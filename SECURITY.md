...
| `{{DEPLOYMENT_PROMPT}}` | PARTIALLY RESOLVED | `REF-SSDF` and `REF-CICD` always apply. Terraform and AWS are explicitly selected in ARCHITECTURE.md and the security notes, so `REF-PROMPT-TF-AWS` and `REF-IAC` apply. The CI/CD platform is GitHub Actions with OIDC federation to AWS IAM roles (selected 2026-07-31), which is the intended delivery mechanism for SEC-CICD-1's short-lived-credential requirement. Container/orchestrator usage and specific AWS services remain UNKNOWN, and the pipeline gate set (SEC-CICD-4) and secret store are TO BE DECIDED (SQ-7). |
...
- **SQ-7** PARTIALLY RESOLVED 2026-07-31 — the CI/CD platform is GitHub Actions, authenticating to AWS by OIDC federation rather than static keys. Still open: the secret-management service, the AWS service topology and environment set, the concrete pipeline design, and which checks block merge under SEC-CICD-4. Least-privilege deployment identities, secret resolution, and IaC scanning cannot be specified concretely until those resolve.
