...
- **Data model expectations:** Relational (RDBMS) (architecture notes). Entities from REQUIREMENTS.md: user account; subscription; consultant engagement; exercise plan and diet plan (each with cited sources and verification record); user plan copy; workout log entry; food log entry; nutrition dataset item (imported at build time, FR-8.11); body weight entry; body measurement entry; consent record; audit entry; session record, email-verification token, MFA enrolment, MFA recovery code, password-reset token, and privileged enrolment invitation. Engine is PostgreSQL and migrations are managed by Drizzle ORM with drizzle-kit (CLAUDE.md, Repository state). Schema design, indexing, and retention behavior: TO BE DECIDED.
- **Deployment model:** Terraform-managed infrastructure on AWS (architecture notes). Services and topology are fixed (SECURITY.md SQ-7 RESOLVED): three environments in separate AWS accounts (dev/staging/production); ECS Fargate for the API, S3 + CloudFront for the client, RDS PostgreSQL Multi-AZ in isolated subnets, Amazon Bedrock in-account for AI inference, AWS Secrets Manager for secrets and signing keys; GitHub Actions deploys via per-environment OIDC roles.
...
- **Relational Persistence (RDBMS)**
  ...
  - **Data owned or accessed:** Stores all entities listed under Data model expectations. Stores no user data that originates outside the system; the bundled nutrition dataset (FR-8.11) is the sole imported external content, ingested at build time as a versioned, integrity-verified artifact.
...
8. *Data export and deletion:* Client → REST API → owner scoping and sensitive-operation authorization → export streamed or hard deletion executed synchronously → audit entry written. Execution is synchronous (SECURITY.md SQ-5 RESOLVED); no export artifact exists, so SEC-DATA-6 remains moot.
