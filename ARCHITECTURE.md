...
- **Client framework:** Vue.js (architecture notes), built by Vite as a single-page application with `vue-router`, written in TypeScript (CLAUDE.md, Repository state). No server-rendering tier exists; the four components below remain the whole system.
