...
- **Server framework:** Node.js (architecture notes), with Fastify as the framework within it and TypeScript as the language (CLAUDE.md, Repository state; selected 2026-07-31).
- **Client framework:** Vue.js (architecture notes), built by Vite as a single-page application with `vue-router`, written in TypeScript (CLAUDE.md, Repository state; selected 2026-08-01). No server-rendering tier exists; the four components below remain the whole system.
...
- **Authentication and session model:** Email + password with optional user-enabled MFA for subscribers; passkey required for admin and consultant accounts (architecture notes; FR-2.2, FR-2.3, FR-2.5, FR-2.6, FR-2.8, FR-2.9). Session transport and revocation were decided 2026-08-01: a signed token carrying a session identifier and no authorization state, in an `HttpOnly`, `Secure`, `SameSite` cookie, resolved against a server-side session record on every request and revoked by invalidating that record (SECURITY.md SEC-SESSION-3, SEC-SESSION-5; SQ-2 RESOLVED). Session lifetimes: TO BE DECIDED (SQ-3).
- **Data model expectations:** Relational (RDBMS) (architecture notes). Entities from REQUIREMENTS.md: user account; subscription; consultant engagement; exercise plan and diet plan (each with cited sources and verification record); user plan copy; workout log entry; food log entry; body weight entry; body measurement entry; consent record; audit entry; and, from the 2026-08-01 authentication decisions, session record, email-verification token, MFA enrolment, and privileged enrolment invitation. Engine is PostgreSQL and migrations are managed by Drizzle ORM with drizzle-kit (CLAUDE.md, Repository state; selected 2026-07-31). Schema design, indexing, and retention behavior: TO BE DECIDED.
...
- **Identity and Session Handling**
  ...
  - **Open decisions:** Whether this is a module of the Node.js application or a separately deployed service — TO BE DECIDED; either way it is a distinct trust boundary. Session transport, revocation, MFA factors, password policy, and privileged provisioning were decided 2026-08-01 (SECURITY.md SEC-SESSION-3, SEC-SESSION-5, SEC-AUTHN-5, SEC-AUTHN-6, SEC-AUTHN-8, SEC-AUTHN-9). Session lifetimes — TO BE DECIDED (SQ-3). Account-recovery and MFA-recovery flows (REQUIREMENTS.md OQ-8) — still UNKNOWN, and the only remaining gap in this component's authentication surface. Credential storage and cryptographic controls are deferred to SECURITY.md.
...
| FR-2.1–FR-2.11 (Accounts and Authentication) | Identity and Session Handling; REST API Application (FR-2.1 enforcement, FR-2.7 role assignment) | SUPPORTED — recovery flows excepted (REQUIREMENTS.md OQ-8) |
