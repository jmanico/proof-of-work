...
- **Relational Persistence (RDBMS)**
  ...
  - **Open decisions:** Schema design, indexing, retention behavior for audit entries, and deletion mechanics satisfying FR-9.4 (hard delete vs. anonymization, and the completion deadline, both TO BE DECIDED in REQUIREMENTS.md) — TO BE DECIDED. Engine is resolved: PostgreSQL with Drizzle ORM and drizzle-kit (see Data model expectations; CLAUDE.md, Repository state). Data residency is resolved (SECURITY.md SQ-1: single US primary region); global-scale replication topology — TO BE DECIDED (SECURITY.md SQ-7).
